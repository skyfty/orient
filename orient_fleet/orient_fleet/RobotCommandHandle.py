...
class RobotCommandHandle(adpt.RobotCommandHandle):
    def __init__(self,
        ...
        self.action_category = None  # The category of the action being executed
        self.action_description = None  # The description of the action being executed
        ...
        self.node.get_logger().info(f"{start.waypoint}")
        ...
        self.node.create_subscription(DockSummary, 'dock_summary',self.dock_summary_cb,qos_profile=transient_qos)
        self.node.create_subscription(ModeRequest, 'action_execution_notice', self.mode_request_cb, qos_profile=qos_profile_system_default)
    ...
    def stop(self):
        ''' This function is used to stop the robot. It will stop the robot
            and clear the current path. The robot will not be able to resume
            the path it was following. The user will need to call
            follow_new_path() to guide the robot along a new path.
        '''
        self.interrupt()
    ...
    def replan(self):
        ''' This function is used to replan the robot's path. It is called when
            the robot detects an obstacle in its path. The robot will stop and
            request a replan from the fleet adapter. The fleet adapter will then
            replan the robot's path and send it back to the robot.
            The robot will then follow the new path.
        '''
        if self.update_handle is not None:
            ...
            self.node.get_logger().info(f'Requesting replan for {self.name} because of an obstacle')

    def follow_new_path(self, waypoints, next_arrival_estimator, path_finished_callback):
        ''' This function is used to guide the robot along a new path. The
            waypoints are expected to be in the RMF coordinate frame and
            should be Plan::Waypoint objects.
        '''
        self.interrupt()
        with self._lock:
            self._follow_path_thread = None
            ...
            def _follow_path():
                ...
                while self.remaining_waypoints  or self.state == RobotState.MOVING:
                    ...
                    if self._quit_path_event.is_set():
                        self.node.get_logger().info(f"[{self.name}] aborting path request")
                        return
                    # State machine
                    if self.state == RobotState.IDLE or target_pose is None:
                        ...
                        response = self.api.navigate(self.name,cmd_id,[x, y, theta],self.map_name,speed_limit)
                        if response:
                            self.remaining_waypoints = self.remaining_waypoints[1:]
                            self.state = RobotState.MOVING
                        else:
                            self.node.get_logger().info(f"Robot {self.name} failed to request navigation to [{x:.0f}, {y:.0f}, {theta:.0f}]. Retrying...")
                        ...
                        with self._lock:
                            if self.api.navigation_completed(self.name, cmd_id):
                                self.node.get_logger().info(f"Robot [{self.name}] has reached the destination for cmd_id {cmd_id}")
                                self.state = RobotState.IDLE
                                ...
                                if lane is not None:
                                    ...
                                    if self.target_waypoint.graph_index is not None and self.dist(self.position, target_pose) < 0.5:
                                        ...
                                        self.on_lane = None
                                        self.on_waypoint = None
                            duration = self.api.navigation_remaining_duration(self.name, cmd_id)
                            if path_index is not None and duration is not None:
                                next_arrival_estimator(path_index,timedelta(seconds=duration))
    ...
    def dock(self, dock_name, docking_finished_callback):
        ...
        with self._lock:
            ...
            def _dock():
                ...
                cmd_id = self.next_cmd_id()
                while not self.api.start_process(self.name, cmd_id, self.dock_name, self.map_name):
                ...
                if self.dock_name not in self.docks:
                    self.node.get_logger().info(f"Requested dock {self.dock_name} not found, ignoring docking request")
                ...
                self.node.get_logger().info(f"Robot {self.name} is docking at {self.dock_name}...")

                while not self.api.process_completed(self.name, cmd_id):
                    if len(positions) < 1:
                        break
                    traj = schedule.make_trajectory(self.vehicle_traits, self.adapter.now(), positions)
    ...
    def update_state(self):
        ...
        with self._lock:
            if (self.on_waypoint is not None):  # if robot is on a waypoint
                self.update_handle.update_current_waypoint(self.on_waypoint, self.position[2])
                ...
                self.update_handle.update_off_grid_position(self.position, self.dock_waypoint_index)
            # if robot is performing an action
                ...
                if not self.started_action:
                    ...
                    if description is None or len(description) < 1:
                        location = self.graph.get_waypoint(self.action_waypoint_index).location
                        description['x'] = location[0]
                        description['y'] = location[1]
                        description['yaw'] = self.position[2]
                    self.api.start_action(self.name, cmd_id, self.action_category, description)
                self.update_handle.update_off_grid_position(self.position, self.action_waypoint_index)
            # if robot is merging into a waypoint
                ...
                self.update_handle.update_off_grid_position(self.position, self.target_waypoint.graph_index)
            else:  # if robot is lost
                self.update_handle.update_lost_position(self.map_name,self.position, max_merge_lane_distance=self.lane_merge_distance)
    ...
    def newly_closed_lanes(self, closed_lanes):
        ...
        if self.target_waypoint is not None and self.target_waypoint.approach_lanes is not None:
            for lane_idx in self.target_waypoint.approach_lanes:
                if lane_idx in closed_lanes:
                    ...
                    if lane_idx == current_lane:
                        ...
                        reverse_lane = self.graph.lane_from(lane.entry.waypoint_index, lane.exit.waypoint_index)
    ...
    def complete_robot_action(self,task_id):
        with self._lock:
    ...
    def mode_request_cb(self, msg):
        if not msg.fleet_name or msg.fleet_name != self.fleet_name or not msg.robot_name or msg.robot_name != self.name:
            return
        if msg.mode.mode == RobotState.IDLE:
            self.complete_robot_action(msg.task_id)
