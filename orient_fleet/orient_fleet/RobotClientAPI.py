...
class RobotAPI:
    ...
    def navigate(self, robot_name: str, cmd_id: int, pose, map_name: str, speed_limit=0.0):
        ...
        assert(len(pose) > 2)
        url = self.prefix + f'/open-rmf/rmf_orient_fm/navigate/?robot_name={robot_name}&cmd_id={cmd_id}'
    ...
    def start_process(self, robot_name: str, cmd_id: int,  process: str, map_name: str):
        ...
        url = self.prefix + f"/open-rmf/rmf_orient_fm/start_task?robot_name={robot_name}&cmd_id={cmd_id}"
    ...
    def stop(self, robot_name: str, cmd_id: int):
        ...
        url = self.prefix + f'/open-rmf/rmf_orient_fm/stop_robot?robot_name={robot_name}&cmd_id={cmd_id}'
        try:
    ...
    def start_action(self, robot_name: str, cmd_id: int, action_category: str, destination: dict):
        '''Request to start a new action for the robot.
           Return True if the action request is successful'''
        url = self.prefix + f"/open-rmf/rmf_orient_fm/start_action?robot_name={robot_name}&cmd_id={cmd_id}"
        data = {}  # data fields: task, map_name, destination{}, data{}
        data['destination'] = destination
        data['task'] = action_category
    ...
    def data(self, robot_name=None):
        if robot_name is None:
            url = self.prefix + f'/open-rmf/rmf_orient_fm/status/'
        else:
            url = self.prefix + f'/open-rmf/rmf_orient_fm/status?robot_name={robot_name}'
        try:
