...
def initialize_fleet(config_yaml, nav_graph_path, node, use_sim_time):
    ...
    fleet_handle.fleet_state_publish_period(datetime.timedelta(seconds=1.0/fleet_state_update_frequency))
    ...
    def _updater_inserter(cmd_handle, update_handle):
        ...
        def _action_executor(category: str, description: dict, execution: adpt.robot_update_handle.ActionExecution):
            with cmd_handle._lock:
                ...
                cmd_handle.action_category = category
        ...
        cmd_handle.update_handle.enable_responsive_wait(cmd_handle.config.get("responsive_wait", False))
        ...
        if (cmd_handle.charger_waypoint_index < cmd_handle.graph.num_waypoints):
            ...
            cmd_handle.node.get_logger().warn( "Invalid waypoint supplied for charger.  Using default nearest charger in the map")
        ...
        while len(missing_robots) > 0:
            ...
            for robot_name in list(missing_robots.keys()):
                ...
                if position is None:
                    ...
                    continue
                node.get_logger().error(f"Unable to determine StartSet for {position[0]} {position[1]}")
                ...
                if robot.initialized:
                    ...
                    # Add robot to fleet
                    fleet_handle.add_robot(robot,robot_name,profile, [starts[0]],partial(_updater_inserter, robot))
    ...
    node.create_subscription(LaneRequest,'lane_closure_requests',_lane_request_cb, qos_profile=qos_profile_system_default)
...
def main(argv=sys.argv):
    ...
    initialize_fleet(config_yaml,nav_graph_path,node, args.use_sim_time)
