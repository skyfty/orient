...
#include "orient_controller/controller_server.hpp"
...
ControllerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  pose_subscription_ = create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>("checkpoint_pose",
        rclcpp::QoS(1),
...
void ControllerServer::computeControl()
{
  if (!active_) {
    ...
    RCLCPP_INFO(get_logger(), "not active");
    return;
  }
...
{
  if (current_pose_) {
    pose.pose = current_pose_->pose.pose;
...
RCLCPP_COMPONENTS_REGISTER_NODE(orient_controller::ControllerServer)
