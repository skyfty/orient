#include "orient_controller/rotation_shim_controller.hpp"
...
{
  position_goal_checker_ = std::make_unique<orient_controller::PositionGoalChecker>();
  ...
  nav2_core::GoalChecker * goal_checker)
{
  ...
  if (path_updated_) {
    ...
    try {
      ...
      geometry_msgs::msg::Pose sampled_pt_base = transformPoseToBaseFrame(pose, getSampledPathPt());
      double angular_distance_to_heading = std::atan2(sampled_pt_base.position.y, sampled_pt_base.position.x);

      double angular_thresh = in_rotation_ ? angular_disengage_threshold_ : angular_dist_threshold_;
...
RotationShimController::transformPoseToBaseFrame(const geometry_msgs::msg::PoseStamped & robot_pose,const geometry_msgs::msg::PoseStamped & pt)
{
  return orient_utils::transform_map_to_baselink(pt.pose, robot_pose.pose);
}
...
void RotationShimController::setPlan(const nav_msgs::msg::Path & path)
{
  ...
  position_goal_checker_->reset();
  // odom_to_map_transform_ = tf_->lookupTransform(current_path_.header.frame_id, costmap_ros_->getGlobalFrameID(), tf2::TimePointZero);
}
...
PLUGINLIB_EXPORT_CLASS(
  orient_controller::RotationShimController,
