...
#include "orient_amcl/amcl_node.hpp"
...
#include "orient_amcl/angleutils.hpp"
#include "nav2_util/geometry_utils.hpp"
#include "orient_amcl/pf/pf.hpp"
#include "nav2_util/string_utils.hpp"
#include "orient_amcl/sensors/laser/laser.hpp"
...
#include "orient_amcl/portable_utils.hpp"
...
: nav2_util::LifecycleNode("amcl", "", options)
{
  ...
  add_parameter("robot_model_type", rclcpp::ParameterValue("orient_amcl::DifferentialMotionModel"));
  ...
  add_parameter(
    "odom_topic", rclcpp::ParameterValue("odometry/laser_scan_matcher"),
    "Topic to subscribe to in order to receive the map to localize on");
  ...
  const rclcpp::Time & sensor_timestamp, const std::string & frame_id)
{
  ...
  //   return false;
  // }
  // odom_pose.header = odom_pose_->header;
  // odom_pose.pose = odom_pose_->pose.pose;

  // x = odom_pose_->pose.pose.position.x;
  // y = odom_pose_->pose.pose.position.y;
  // yaw = tf2::getYaw(odom_pose_->pose.pose.orientation);
  // return true;
  ...
  const pf_vector_t & pose)
{
  ...
  lasers_[laser_index]->sensorUpdate(pf_, reinterpret_cast<orient_amcl::LaserData *>(&ldata));
...
AmclNode::createLaserObject()
{
  ...
  return new orient_amcl::LikelihoodFieldModel(
    z_hit_, z_rand_, sigma_hit_,
...
void
AmclNode::odomReceived(const nav_msgs::msg::Odometry::SharedPtr msg)
{
  odom_pose_ = msg;
...
AmclNode::initPubSub()
{
  ...
  odometry_subscription_ = create_subscription<nav_msgs::msg::Odometry>(
    ...
    std::bind(&AmclNode::odomReceived, this, std::placeholders::_1));
...
RCLCPP_COMPONENTS_REGISTER_NODE(orient_amcl::AmclNode)
