...
#include "orient_gazebo_plugins/gz_ros2_control_plugin.hpp"
#include "orient_gazebo_plugins/gz_system.hpp"

namespace orient_gazebo_plugins
{
//////////////////////////////////////////////////
...
public:
  ...
  std::shared_ptr<pluginlib::ClassLoader<
      orient_gazebo_plugins::GazeboSimSystemInterface>>
      ...
      case sdf::JointType::FIXED:
        {
          RCLCPP_INFO(
            node_->get_logger(),
            "[orient_gazebo_plugins] Fixed joint [%s] (Entity=%lu)] is skipped",
      ...
      case sdf::JointType::UNIVERSAL:
        {
          RCLCPP_WARN(
            node_->get_logger(),
            "[orient_gazebo_plugins] Joint [%s] (Entity=%lu)] is of unsupported type."
      ...
      default:
        {
          RCLCPP_WARN(
            node_->get_logger(),
            "[orient_gazebo_plugins] Joint [%s] (Entity=%lu)] is of unknown type",
            jointName.c_str(), jointEntity);
          continue;
        }
    }
...
std::string GazeboSimROS2ControlPluginPrivate::getURDF() const
{
  ...
  while (urdf_string.empty()) {
    ...
    if (!urdf_string.empty()) {
      ...
      RCLCPP_ERROR(
        node_->get_logger(), "orient_gazebo_plugins plugin is waiting for model"
  ...
  sim::EventManager &)
{
  ...
  if (!model.Valid(_ecm)) {
    RCLCPP_ERROR(
      logger,
      "[orient_gazebo_plugins] Failed to initialize because [%s] (Entity=%lu)] is not a model."
      "Please make sure that orient_gazebo_plugins is attached to a valid model.",
  ...
  if (paramFileName.empty()) {
    RCLCPP_ERROR(
      logger,
      "orient_gazebo_plugins found an empty parameters file. Failed to initialize.");
    return;
  }

  RCLCPP_INFO(
    logger,
    "dddddddddddddddddddddddddddddddddddddddddd is %s", paramFileName.c_str());
  // Get params from SDF
  ...
  while (argument_sdf) {
    ...
    static const size_t package_prefix_len = sizeof(package_prefix) - 1;
    size_t found = argument.find(package_prefix);
    if (found == 0) {
      found = argument.find("/", package_prefix_len);
      std::string package_name = argument.substr(package_prefix_len, found - package_prefix_len);
      std::string package_share_directory = ament_index_cpp::get_package_share_directory(package_name);
      argument.replace(0, found, package_share_directory);
    }
  ...
  std::string node_name = "orient_gazebo_plugins";
  ...
  RCLCPP_DEBUG_STREAM(
    this->dataPtr->node_->get_logger(), "[orient_gazebo_plugins] Setting up controller for [" <<
  ...
  if (enabledJoints.size() == 0) {
    RCLCPP_DEBUG_STREAM(
      ...
      "[orient_gazebo_plugins] There are no available Joints.");
    return;
  }
  ...
  try {
    ...
    RCLCPP_ERROR_STREAM(
      this->dataPtr->node_->get_logger(),
      "Error parsing URDF in orient_gazebo_plugins plugin, plugin not active : " << ex.what());
    return;
  }
  ...
  try {
    this->dataPtr->robot_hw_sim_loader_.reset(
      new pluginlib::ClassLoader<orient_gazebo_plugins::GazeboSimSystemInterface>(
        "orient_gazebo_plugins",
        "orient_gazebo_plugins::GazeboSimSystemInterface"));
  ...
  for (unsigned int i = 0; i < control_hardware_info.size(); ++i) {
    ...
    std::unique_ptr<orient_gazebo_plugins::GazeboSimSystemInterface> gzSimSystem;
    RCLCPP_INFO(
      this->dataPtr->node_->get_logger(), "Load hardware interface %s ...",
      robot_hw_sim_type_str_.c_str());

    try {
      gzSimSystem = std::unique_ptr<orient_gazebo_plugins::GazeboSimSystemInterface>(
  ...
  const sim::EntityComponentManager & /*_ecm*/)
{
  ...
  if (sim_period >= this->dataPtr->control_period_) {
    ...
    auto gz_controller_manager =
      std::dynamic_pointer_cast<orient_gazebo_plugins::GazeboSimSystemInterface>(
...
IGNITION_ADD_PLUGIN(
  orient_gazebo_plugins::GazeboSimROS2ControlPlugin,
  sim::System,
  orient_gazebo_plugins::GazeboSimROS2ControlPlugin::ISystemConfigure,
  orient_gazebo_plugins::GazeboSimROS2ControlPlugin::ISystemPreUpdate,
  orient_gazebo_plugins::GazeboSimROS2ControlPlugin::ISystemPostUpdate)